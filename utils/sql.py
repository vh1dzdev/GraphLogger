    ...
    async def _init():
        if os.path.isfile(cfg.db_path):
            ...
            async with aiosqlite.connect(cfg.db_path) as db:
                await db.execute("CREATE TABLE IF NOT EXISTS loggers (id STRING PRIMARY KEY, logger_id TEXT)")
                await db.execute("CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY, ip TEXT, logger_id TEXT, date TEXT)")
                await db.commit()
